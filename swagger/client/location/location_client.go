...
func (a *Client) LocationListHTTP(params *LocationListHTTPParams, authInfo runtime.ClientAuthInfoWriter, opts ...ClientOption) (*LocationListHTTPOK, error) {
	...
	if params == nil {
		params = NewLocationListHTTPParams()
	}
	op := &runtime.ClientOperation{
		ID:                 "locationListHttp",
		Method:             "GET",
		PathPattern:        "/provisioning/v1/location",
		ProducesMediaTypes: []string{"application/json"},
		ConsumesMediaTypes: []string{"application/json"},
		Schemes:            []string{"https"},
		Params:             params,
		Reader:             &LocationListHTTPReader{formats: a.formats},
		AuthInfo:           authInfo,
	...
	success, ok := result.(*LocationListHTTPOK)
	if ok {
		return success, nil
	}
	// unexpected success response
	// safeguard: normally, absent a default response, unknown success responses return an error above: so this is a codegen issue
	msg := fmt.Sprintf("unexpected success response for locationListHttp: API contract not enforced by server. Client expected to get an error, but got: %T", result)
	panic(msg)
}
